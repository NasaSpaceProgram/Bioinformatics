...
def ReadSequencesFromMultisequenceFASTA(filename, DiscriptorKey = "orf"):
    """Reads from fasta file then outputs only the sequences as a list of unbroken strings
        DiscriptorKey: the first charicters are for each discriptor; helps differentiate each sequence
        discriptor: tells funciton wheather or not to includ the discriptor """
    f = open(filename, "r")
    lines = f.readlines()
    f.close()
    ...
    curr_sequence = []
    for line in lines[1:]:
        if line[:len(DiscriptorKey)] == DiscriptorKey:
            stringSeq = "".join(curr_sequence)
            stringSeq = stringSeq.replace("\n","")
...
def WrightSequenceToFASTA(filename, Description = "", Sequence = "", lineSize = 60, writetype = "w"):
    ...
    lineSize: number of charicters on each line, i.e number of charicters befor it puts an \n. Only applicable if Sequence is a string
              if lineSize is False then it will simply print the sequence as a line
...
def find_reading_frames(DNA, MinCodons = 100, StartCodons = ["ATG"], StopCodons = ["TAG","TAA","TGA"]):
    ...
    MinCodons: Minimum # of codons allowed in a reading frame
